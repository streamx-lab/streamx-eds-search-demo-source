...
function expandSelection(parent) {
  ...
  const element = document.getElementById(`sidenav-li-${id}`);
  if (!element) return;
...
function addResizeBar() {
  ...
  div.addEventListener('mousedown', (evt) => {
    ...
    document.addEventListener('mousemove', (event) => {
      if (isResizing) {
        const newWidth = event.pageX - sidenavContainer.offsetLeft;
...
function createTree(parent, data, prefix, level) {
  ...
  data.forEach((item, idx) => {
    ...
    const siteURL = `${window.location.protocol
    }//${
      window.location.hostname
    }${window.location.port ? `:${window.location.port}` : ''}`;
    if (item.url) {
      const navURL = new URL(item.url, siteURL).href;
