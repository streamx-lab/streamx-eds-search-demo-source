...
const minitocContainer = content.getElementsByClassName('minitoc')[0];
const miniTOCList = document.createElement('ul');
...
if (tagList.length !== 0) {
  const h2minitocText = document.createElement('h2');
  h2minitocText.classList.add('minitoc-title');
  const textNode = document.createTextNode('In this article');
...
tagList.forEach((tag) => {
  tag.addEventListener('click', (event) => {
    ...
    setTimeout(() => {
      ...
      if (a_tag) {
        const href = a_tag.attributes.href.value;
        let target_id;
        if (href) {
        ...
        for (let i = 0; i < headings.length; i++) {
          const heading = headings[i];
          const id = heading.attributes.id.value;
          if (id === target_id) {
            targetNode = heading;
            break;
          }
